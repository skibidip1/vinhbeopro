...
local TabFarm = Window:AddTab({ Title = "Auto Farm", Icon = "swords" })
local TabMaterial = Window:AddTab({ Title = "Material", Icon = "package" })
local TabFastAttack = Window:AddTab({ Title = "Fast Attack", Icon = "zap" })

-- Fast Attack Toggle
TabFastAttack:AddToggle("FastAttackToggle", {
    Title = "Enable Fast Attack",
    Description = "Tăng tốc độ đánh quái",
    Default = _G.FastAttack or false,
    Callback = function(state)
        _G.FastAttack = state
        if state and not _ENV.rz_FastAttack then
            print("[FastAttack] Đã bật - cần re-execute nếu không tự chạy.")
        end
    end
})
...
-- Material Dropdown
TabMaterial:AddDropdown("SelectMaterialDropdown", {
    Title = "Chọn Material",
    Description = "Chọn nguyên liệu để farm",
    Values = {
        "Radiactive Material", "Leather + Scrap Metal", "Magma Ore", "Fish Tail",
        "Angel Wings", "Mystic Droplet", "Vampire Fang", "Gunpowder",
        "Mini Tusk", "Conjured Cocoa"
    },
    Multi = false,
    Default = 1,
    Callback = function(selected)
        _G.SelectMaterial = selected
    end
})
